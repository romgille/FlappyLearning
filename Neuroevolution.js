...
var Neuroevolution = function (options) {
    ...
    Network.prototype.getSave = function () {
        ...
        for (var i in this.layers) {
            datas.neurons.push(this.layers[i].length);
            for (var j in this.layers[i]) {
                for (var k in this.layers[i][j].weights) {
                    // push all input weights of each Neuron of each Layer into a flat
                    // array.
                    datas.weights.push(this.layers[i][j].weights[k]);
                }
            }
        }
        return datas;
        ...
        for (var i in save.neurons) {
            ...
            for (var j in layer) {
                for (var k in layer[j].weights) {
                    ...
                    layer[j].weights[k] = save.weights[indexWeights];
    ...
    Network.prototype.compute = function (inputs) {
        ...
        for (var i in inputs) {
            if (this.layers[0] && this.layers[0][i]) {
